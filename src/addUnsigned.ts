...
export default function addUnsigned(a: DecimalInfo, b: DecimalInfo, minScale: number = 0): DecimalInfo {
    const maxScale = max(a.scale, b.scale);
    const maxLength = max(a.length, b.length) + 1;
    const result = createInfo(maxLength, max(maxScale, minScale));
    ...
    let tmp;
    for (
        ; aLengthLeft > 0 && bLengthLeft > 0
        ; aPos -= 1, bPos -= 1, resultPos -= 1, aLengthLeft -= 1, bLengthLeft -= 1
    ) {
        // add the two numbers together
        tmp = a.value[aPos] + b.value[bPos] + carry;
        // check if they are >= 10 (impossible to be more then 18)
        if (tmp >= DECIMAL_RADIX) {
            carry = 1;
            tmp -= DECIMAL_RADIX; // yep, subtract 10, add a carry
        } else {
            carry = 0;
        }
